...
def buy():
    ...
    if request.method == "GET":
        ...
        try:
            shares = int(shares)  # Try to convert shares to an integer
            if shares <= 0:
                raise ValueError("Shares must be a positive integer.")  # Catch negative and zero shares
        except ValueError:
            flash("Invalid number of shares. Please enter a positive integer.", "error")
            return render_template("buy.html")
